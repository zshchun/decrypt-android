...
set "ANDROID_LAB_PATH=%ANDROID_LAB_BIN%;%ANDROID_LAB_VENV%\Scripts;%ANDROID_SDK_ROOT%\platform-tools;%ANDROID_SDK_ROOT%\emulator;%ANDROID_SDK_ROOT%\cmdline-tools\latest\bin;%ANDROID_NDK_ROOT%\toolchains\llvm\prebuilt\windows-x86_64\bin"
...
:Persist
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $names=@('ANDROID_LAB_ROOT','ANDROID_LAB_VENV','ANDROID_HOME','ANDROID_SDK_ROOT','ANDROID_NDK_ROOT'); foreach ($name in $names) { [Environment]::SetEnvironmentVariable($name,[Environment]::GetEnvironmentVariable($name,'Process'),'User') }; $entries=@($env:ANDROID_LAB_BIN, (Join-Path $env:ANDROID_LAB_VENV 'Scripts'), (Join-Path $env:ANDROID_SDK_ROOT 'platform-tools'), (Join-Path $env:ANDROID_SDK_ROOT 'emulator'), (Join-Path $env:ANDROID_SDK_ROOT 'cmdline-tools\latest\bin'), (Join-Path $env:ANDROID_NDK_ROOT 'toolchains\llvm\prebuilt\windows-x86_64\bin')); $legacyNdkRoot=Join-Path $env:ANDROID_SDK_ROOT 'ndk\21.0.6113669'; $removedEntries=@((Join-Path $legacyNdkRoot 'prebuilt\windows-x86_64\bin'), (Join-Path $legacyNdkRoot 'toolchains\llvm\prebuilt\windows-x86_64\bin')); if ($env:JAVA_HOME) { $entries += (Join-Path $env:JAVA_HOME 'bin') }; $userPath=[Environment]::GetEnvironmentVariable('Path','User'); $parts=@($userPath -split ';' | Where-Object { $_ -and ($entries -notcontains $_) -and ($removedEntries -notcontains $_) }); $newPath=($entries + $parts) -join ';'; [Environment]::SetEnvironmentVariable('Path',$newPath,'User')"
